...
- **Known gap this feature does NOT close (found while building it, out of scope to fix here):** `RecommendationHistory.GetExclusions()` / `GetExclusionPrompt()` have **no production caller that feeds them into prompt-building or post-generation filtering** — nothing in the recommendation pipeline (`RecommendationPipeline`, `SafetyGateService`, `DuplicationPreventionService`) reads `Disliked`/`StronglyDisliked` today. So "Never again" currently writes an inert record: it does not yet stop a disliked artist from being re-suggested by a live run. `exclusions/remove` correctly reverses whatever `review/never` did (proven by round-trip tests against `GetExclusions()`, the only query surface that exists), but wiring the exclusion store into live generation is a separate, larger change and was deliberately left out here — do not read this section as "Never again enforces anything today."
