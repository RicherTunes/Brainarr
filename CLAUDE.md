...
Audited sound 2026-05-30: all use `Authorization: Bearer`, correct per-provider endpoints, `max_tokens` from the timeout-aware `request.MaxTokens` budget (never hardcoded), null-safe `choices[0].message.content` parsing, and `temperature` (correct for OpenAI format — opposite of ZaiCoding). **Shared adoption (2026-09-07): all six (incl. ZaiGlm) now derive directly from Common's `OpenAiChatProviderBase`**, which owns request serialization, completion parsing, health probes, auth-circuit flow, streaming decode, and timeout policy. Brainarr retains thin Lidarr HTTP/streaming and auth-circuit adapters; provider quirks remain visible in each provider's hooks (OpenRouter's `HTTP-Referer` + `X-Title` headers and cheap probe model, Perplexity's citation stripping + no `response_format`, ZaiGlm's `MapZaiHttpError`, OpenAI's 0.8 default temperature + REQUEST_START logging scope). Common's contract suite pins the shared behavior, while `SharedOpenAiChatAdoptionTests` enters through the real registry/factory and pins the six direct subclasses, removal of the old local base, distinct wire contracts, and malformed-content failures. Deliberately NOT on the base: `BrainarrZaiCodingProvider` (Anthropic wire format, raw HttpClient, opposite temperature contract), `BrainarrOpenAiCompatibleProvider` (optional auth + conditional circuit, GET `/v1/models` health check with Degraded semantics), Anthropic, Gemini, and Codex Responses.

### OpenAI Codex Subscription (`BrainarrOpenAiCodexSubscriptionProvider`) — dual-mode: ChatGPT backend + API-key fallback

**The ChatGPT-subscription path now works** (live-verified 2026-08 by a Plus subscriber against a real Lidarr 3.1.2.4913 host: Test green, sync returns recommendations). The provider is dual-mode, chosen by `SubscriptionCredentialLoader.LoadCodexCredentials`:

- **`chatgpt` mode** (a `codex auth login` session — `tokens.access_token`, no usable `OPENAI_API_KEY`): the OAuth token is **not** accepted by the Platform `chat/completions` API. Requests go to the ChatGPT backend Responses API instead: `POST https://chatgpt.com/backend-api/codex/responses` with `Authorization: Bearer` + `chatgpt-account-id` + `OpenAI-Beta: responses=experimental` + `originator: codex_cli_rs` + a `codex_cli_rs/<ver>` UA + `session_id` + `Accept: text/event-stream`. Dispatched through a **raw `HttpClient`** (not Lidarr's `IHttpClient`) because `ManagedHttpDispatcher` throws on the non-Lidarr UA/originator the backend requires — same rationale and shape as `BrainarrZaiCodingProvider`. Response is SSE, reconstructed by `CodexSseParser`.
- **`apikey` mode** (an `OPENAI_API_KEY` in `auth.json`): unchanged legacy path through Lidarr's `IHttpClient` to `api.openai.com/v1/chat/completions`.

Hard-won gotchas — each was a live 400/401 or a UI dead-end, all easy to regress:

- **`stream: true` is REQUIRED.** The backend does not answer a non-streaming Responses request.
- **NEVER send `max_output_tokens`.** This backend rejects it outright: `400 {"detail":"Unsupported parameter: max_output_tokens"}`. So `request.MaxTokens` — the timeout-aware output budget every *other* provider honours — **cannot** be passed here and is deliberately dropped. Adding it "for consistency" breaks every request. Consequence: output length is bounded only by the request timeout, so a verbose run hits the deadline with nothing to salvage; the mitigation is the user-facing **AI Request Timeout**, which must be raised to **60s+** (the live 30s default timed out at 0 items). The timeout message says so.
- **Model slugs are backend-specific and plan-gated.** Only `BrainarrConstants.OpenAICodexModels` (`gpt-5.6-terra`/`-sol`/`-luna`, `gpt-5.5`) are accepted on a Plus account; bare `gpt-5.6`, `gpt-5.6-codex`, `codex-mini-latest`, `gpt-5.3-codex-spark` (Pro-only) and every Platform slug (`gpt-4o`, `o3`, …) return `400 "model is not supported when using Codex with a ChatGPT account"`. `gpt-5.4`/`gpt-5.4-mini` answer 200 today but **leave Codex 2026-08-31**, so they're excluded on purpose — match against the known-good set, **not** a `gpt-5` prefix. `GET /backend-api/codex/models` returns `{"models":[]}` for a subscription account, so the dropdown is static, not discovered.
- **`NormalizeCodexModel` is load-bearing for the settings UI, not mere defensiveness.** Lidarr does **not** refetch the field schema when only the *Provider* dropdown changes (no cross-field trigger exists in `FieldDefinition` — the only refetch triggers the frontend fires are editing a credential field and a successful Test). So switching to Codex leaves the previous provider's model in the field (e.g. `GPT41_Mini`) → 400 → **Test fails → Lidarr refuses to Save** → the user can never save Codex to *get* the refreshed dropdown. Coercing an unknown slug to the default breaks that deadlock: Test passes, Save works, correct models appear on reopen. The coercion logs a Warn — silently discarding an explicit pick is invisible in support logs.
- **Do NOT advertise `LlmCapabilityFlags.JsonMode` here.** `text.format={type:json_object}` *is* supported, but only when the **input message** itself contains the word "json" — otherwise `400 "Response input messages must contain the word 'json'"`. That's prompt-dependent, so it can't be guaranteed; advertising the flag while dropping it on the wire would make the pipeline believe strict JSON is enforced when nothing enforces it. System-prompt JSON shaping already yields a clean array from these models.
- **`reasoning` effort does not help.** `effort: none` measured 12.6s vs 14.8s unset for a full recommendation list — the cost is generation + network, not a reasoning preamble. (`minimal` is rejected; the accepted set is `none|low|medium|high|xhigh|max`.)

**Token rotation** (`CodexTokenRefresher` + `CodexJwt`): the access token is a **~10-day JWT** and `auth.json` carries **no `expires_at`** — the lifetime lives in the JWT `exp` claim, which is why `LoadCodexCredentials` falls back to decoding it. Refresh is the OAuth2 refresh-token grant against `https://auth.openai.com/oauth/token` with the Codex CLI's public client id, writing the rotated `access_token`/`id_token`/`refresh_token` back into `~/.codex/auth.json` **atomically** (temp + `File.Move`, `0600` on POSIX) so a crash can't leave the CLI with a truncated auth file; every other field (`auth_mode`, `OPENAI_API_KEY`, `tokens.account_id`) is preserved. Refresh is **reactive only** — 401/403 → refresh once → retry — and is wired on **both** `CompleteAsync` and `CheckHealthAsync`: without it on the health path a Test taken after expiry goes red while a concurrent sync succeeds, and since Lidarr blocks Save on a failed Test that strands the user again. `CredentialRefreshService` (proactive timer) also calls the refresher, but note that service still has **no production caller** — don't cite it as the live path.
- **`auth_mode` in the file is not authoritative.** Reaching the `tokens.access_token` branch means the only credential present is an OAuth token, so the mode is forced to `chatgpt` regardless of the label. A file can legitimately read `auth_mode:"apikey"` with `OPENAI_API_KEY:null` (the CLI writes that when the key comes from the environment); copying that verbatim would route an OAuth bearer to the Platform endpoint and 401 on every request — the original day-one bug.
- **A `response.failed`/`error` SSE event on an HTTP 200 must throw**, not return an empty success: `CodexSseParser` captures it in `ErrorDetail` and the provider raises when there's no usable text. Returning empty-success would log "0 recommendations" with no cause *and* call `RecordSuccess` on the auth circuit for what may be a credential/quota problem.

**Lesson (supersedes the earlier "don't blind-fix" note):** the previous audit was right that a blind rewrite was unsafe — the fix only became safe once the endpoint, headers, body and model set were **probed live** against a real subscription and pinned with tests. Probe first, then pin.
