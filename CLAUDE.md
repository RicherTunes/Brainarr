...
**Regression guard**: `Brainarr.Tests/Services/Core/AlbumArtistLazyLoadTestDoubles.cs` provides `RecordingArtistLazyLoaded` (a public `LazyLoaded<Artist>` subclass standing in for the real internal lazy-load proxy) + `LazyLoadCounter`, used by `DuplicateFilterServiceNPlusOneTests`, `LibraryContextBuilderNPlusOneTests`, and `StyleContextBuilderNPlusOneTests` to assert the per-album DB round trip count is **0** regardless of album count (proven: unfixed code hit exactly 2,000 round trips for 2,000 albums; fixed code hits 0), and that a per-row query *failure* (simulating the live `Links` column-21 fault) no longer collapses `BuildProfile` to the fallback profile. **Any future code that reads `Album.ArtistId`/`Album.Artist.Value` inside a loop over `IAlbumService.GetAllAlbums()` results will fail these tests** — that's the intended guard; batch via `ArtistMetadataId` instead. A runtime cap/circuit-breaker was considered and deliberately not added: the batch-load fix is a single O(artists + albums) pass, inherently bounded at any library size (no cap needed), and the existing outer try/catch + fallback profile in `LibraryContextBuilder`/`BrainarrOrchestrator` already provides graceful degradation for any *other* unforeseen failure on this path.

**Known follow-up (same bug pattern, not yet fixed — out of scope for this pass):** `Album.ArtistId` is read in a loop in several other places that were not on the reported crash's call path: `LibraryAnalyzer.cs` (`AnalyzeCollectionDepth`, `GetTopArtistsByAlbumCount`, and its own `LibraryProfile`-vs-`BuildProfile` -esque top-collected-artist name resolution), `LibraryContextBuilder`'s sibling `LibraryPromptPlanner.cs`/`LibraryPromptRenderer.cs` (`.ThenBy(a => a.ArtistId)`), `DefaultSamplingService.cs` (multiple sites), `AdvancedDuplicateDetector.cs`, and `SimpleRecommendationValidator.cs` (both: `albums.Where(a => a.ArtistId == artist.Id)`). Each independently calls (or receives) a fresh `GetAllAlbums()`/`GetAllArtists()` result, so each is its own potential N+1 site. They should be swept to the same `ArtistMetadataId` pattern in a follow-up pass.
