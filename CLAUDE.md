...
### Common helpers intentionally not adopted (architectural divergence)

- **`AuthFailureGate` / `AuthFailureGateRegistry`** — Brainarr uses its own `LlmAuthCircuit` (`Brainarr.Plugin/Services/Resilience/LlmAuthCircuit.cs`) instead. Same purpose (fail-fast when auth is known bad to stop hammering the upstream), different semantics, by design:
  - **Key hashing**: `LlmAuthCircuit.MakeKey` stores `{providerId}::{SHA256(apiKey)[0..16]}` so a raw LLM API key never appears as a dict key — eliminates a heap-dump / debugger-inspection leak vector that matters more for LLM keys (broad permissions, billing-attached) than for streaming-session tokens. `AuthFailureGateRegistry` would need to either match this hashing on the call site or grow its own opt-in hashing helper.
  - **Sliding window**: 3 consecutive failures within a 5-min sliding window opens the circuit. Failures outside the window are forgotten so a stale 401-run against a now-rotated key doesn't pre-latch the new credential. Common's `DefaultAuthFailureHandler` is K-consecutive-without-reset; the K-1 sub-threshold flips the gate into rate-limit mode, which is the streaming-service pattern but creates avoidable friction when LLM providers hiccup during token-refresh races (Anthropic OAuth rotation, OpenAI key propagation delays).
  - **30-min Open duration → HalfOpen single probe → Open-or-Closed**: matches the human-recoverable revocation pattern (user fixes the key in dashboard, restarts plugin or waits). Common's gate uses a continuous 60s probe interval — fine for streaming-session token refresh, too aggressive for LLM key rotation where the recovery cycle is minutes-to-hours.
  - **Coverage**: `LlmAuthCircuit` is currently wired in only 3 of 11 cloud providers (OpenAI, Anthropic, ClaudeCodeSub). The other 8 don't have circuit coverage yet — a separate parity gap inside Brainarr, not an ecosystem one.
  - **Convergence path**: extending `Common.DefaultAuthFailureHandler` (or adding a `SlidingWindowAuthFailureHandler` sibling) to support `(failureThreshold, failureWindow, openDuration)` would unblock Brainarr-side adoption while preserving its semantics. Not done in the current session — tracked as a Common extension opportunity. The user's mission contract explicitly classifies the 11-provider LLM matrix as "different by design" so this is a documented N/A rather than tech debt for now.
