...
Audited sound 2026-05-30: all use `Authorization: Bearer`, correct per-provider endpoints, `max_tokens` from the timeout-aware `request.MaxTokens` budget (never hardcoded), null-safe `choices[0].message.content` parsing, and `temperature` (correct for OpenAI format — opposite of ZaiCoding). **Dedup #46 landed (2026-07-16): all six (incl. ZaiGlm) now derive from `BrainarrOpenAiChatProviderBase`**, which owns the shared `BuildRequestBody`/`SendAsync`/`ParseCompletion`/auth-circuit/health-probe/streaming plumbing; per-provider quirks live in template-hook overrides in each provider file (OpenRouter's `HTTP-Referer` + `X-Title` headers and cheap probe model, Perplexity's citation stripping + no `response_format`, ZaiGlm's `MapZaiHttpError`, OpenAI's 0.8 default temperature + REQUEST_START debug line). The base's hook contract is pinned by `OpenAiChatProviderBaseContractTests` (incl. the `Temperature_OmittedWhenPolicySaysNo` policy hook and an exact-JSON body-shape pin); the per-provider `CompleteAsync_UsesBearerAuth_Against…Endpoint` pins remain. Deliberately NOT on the base: `BrainarrZaiCodingProvider` (Anthropic wire format, raw HttpClient, opposite temperature contract) and `BrainarrOpenAiCompatibleProvider` (optional auth + conditional circuit, GET `/v1/models` health check with Degraded semantics, ProviderOptions-based opt-in JSON mode, no streaming).
