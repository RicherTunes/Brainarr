...
- **Confidence floor (`MinConfidence`) gotcha.** Now a visible advanced setting (default 0.7), applied by `SafetyGateService` (drop, or queue when `QueueBorderlineItems`), validated to `[0,1]`. **Trap:** `MinimalResponseParser` fabricates `confidence=0.7` when the model omits it, so raising the floor above 0.7 silently drops every score-less recommendation. It's also read by `RecommendationTriageAdvisor` (triage reason codes), so it's more far-reaching than drop/queue. Help text warns about the cliff; a deeper fix (distinct "unknown" sentinel + targeted under-target hint) is queued.
