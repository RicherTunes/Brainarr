# Archived Brainarr Documentation

These documents are kept for historical reference. They document past planning, audits, and decisions that shaped the current implementation.

> **Note:** For current documentation, see the [main docs README](../README.md).

## Documentation Audits

Historical audits that drove documentation improvements:

| Document | Purpose |
|----------|---------|
| [Comprehensive Audit Report](COMPREHENSIVE_DOCUMENTATION_AUDIT_REPORT.md) | Full documentation review |
| [Audit Complete](DOCUMENTATION_AUDIT_COMPLETE.md) | Audit completion sign-off |
| [Audit Final Report](DOCUMENTATION_AUDIT_FINAL_REPORT.md) | Conclusions and recommendations |
| [Audit Report](DOCUMENTATION_AUDIT_REPORT.md) | Initial audit findings |
| [Audit Summary](DOCUMENTATION_AUDIT_SUMMARY.md) | Executive summary of audit |
| [Enhancement Report](DOCUMENTATION_ENHANCEMENT_REPORT.md) | Implemented improvements |
| [GitHub Ready Summary](GITHUB_READY_SUMMARY.md) | Pre-release documentation checklist |

## Technical Debt & Remediation

Planning documents for code quality improvements:

| Document | Purpose |
|----------|---------|
| [Tech Debt Analysis](TECH_DEBT_ANALYSIS_REPORT.md) | Identified technical debt |
| [Remediation Plan](TECHNICAL_DEBT_REMEDIATION_PLAN.md) | Planned fixes |
| [Remediation Report](TECH_DEBT_REMEDIATION_REPORT.md) | Completed fixes |

## Migration & Refactoring

| Document | Purpose |
|----------|---------|
| [Refactoring Migration Guide](REFACTORING_MIGRATION_GUIDE.md) | Code restructuring guidance |
| [Security Improvements](SECURITY_IMPROVEMENTS.md) | Security enhancements made |

## Testing

| Document | Purpose |
|----------|---------|
| [Test Fix Summary](TEST_FIX_SUMMARY.md) | Test suite improvements |

## Superseded Documentation

These documents were replaced by current versions:

| Archived | Replaced By |
|----------|-------------|
| [Troubleshooting Enhanced](TROUBLESHOOTING_ENHANCED.md) | [troubleshooting.md](../troubleshooting.md) |

---

*Last organized: March 2026*
