...
namespace NzbDrone.Core.ImportLists.Brainarr.Services
{
    public class PerplexityProvider : IAIProvider
    {
        ...
        public PerplexityProvider(IHttpClient httpClient, Logger logger, string apiKey, string model = "llama-3.1-sonar-large-128k-online", bool preferStructured = true, NzbDrone.Core.ImportLists.Brainarr.Services.Resilience.IHttpResilience? httpExec = null)
        {
            ...
            if (_httpExec == null)
            {
                try { _logger.WarnOnceWithEvent(12001, "PerplexityProvider", "PerplexityProvider: IHttpResilience not injected; using static resilience fallback"); } catch (Exception) { /* Non-critical */ }
            }
        }
        ...
        private async Task<List<Recommendation>> GetRecommendationsInternalAsync(string prompt, System.Threading.CancellationToken cancellationToken)
        {
            try
            {
                ...
                foreach (var body in attempts)
                {
                    ...
                    if (response == null)
                    {
                        try { _logger.Debug("Perplexity request returned null response; trying next fallback body"); } catch (Exception) { /* Non-critical */ }
                        idx++; continue;
                    }
                    var code = (int)response.StatusCode;
                    if (response.StatusCode == System.Net.HttpStatusCode.BadRequest || code == 422)
                    {
                        try { _logger.Debug($"Perplexity rejected body format with {code}; falling back (attempt {idx + 1})"); } catch (Exception) { /* Non-critical */ }
                        idx++; continue;
                    }
                    usedIndex = idx;
                    break;
        ...
        public async Task<bool> TestConnectionAsync()
        {
            ...
            try
            {
                ...
                if (!success)
                {
                    TryCapturePerplexityHint(response.Content, (int)response.StatusCode);
                }
        ...
        private void TryCapturePerplexityHint(string? body, int status)
        {
            try
            {
                _lastUserMessage = null;
                ...
                if (status == 401 || content.IndexOf("invalid_api_key", StringComparison.OrdinalIgnoreCase) >= 0 || content.IndexOf("unauthorized", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    _lastUserMessage = "Invalid Perplexity API key. Verify your key at https://www.perplexity.ai/settings/api and ensure it is active.";
                    _lastUserLearnMoreUrl = BrainarrConstants.DocsPerplexitySection;
                }
                else if (status == 429 || content.IndexOf("rate limit", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    _lastUserMessage = "Perplexity rate limit exceeded. Wait a few minutes or reduce request frequency.";
                    _lastUserLearnMoreUrl = BrainarrConstants.DocsPerplexitySection;
                }
                ...
                {
                    _lastUserMessage = "Perplexity quota exhausted. Check your subscription at https://www.perplexity.ai/settings/api.";
                    _lastUserLearnMoreUrl = BrainarrConstants.DocsPerplexitySection;
                }
            }
            catch (Exception) { /* Non-critical */ }
