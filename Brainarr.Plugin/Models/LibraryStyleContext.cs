...
namespace NzbDrone.Core.ImportLists.Brainarr.Models
{
    ...
    public sealed class LibraryStyleContext
    {
        ...
        {
            DominantStyles = dominant?.Where(s => !string.IsNullOrWhiteSpace(s)).ToArray() ?? Array.Empty<string>();
        }
    ...
    public sealed class LibrarySampleArtist
    {
        ...
        public string[] MatchedStyles { get; init; } = Array.Empty<string>();
    ...
    public sealed class LibraryStyleIndex
    {
        ...
        private static IReadOnlyList<int> Combine(IEnumerable<string> styleSlugs, IReadOnlyDictionary<string, IReadOnlyList<int>> source)
        {
            ...
            var estimated = 0;
            foreach (var slug in filtered)
            {
                if (source.TryGetValue(slug, out var ids) && ids != null)
                {
                    estimated += ids.Count;
                }
            }
            ...
            var unique = new HashSet<int>(estimated, EqualityComparer<int>.Default);
