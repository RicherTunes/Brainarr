...
namespace Brainarr.Tests.Services.Providers.Contracts.SecurityContractTests
{
    ...
    public class PerplexitySecurityContractTests : IDisposable
    {
        ...
        {
            var innerException = new Exception("Connection failed with key pplx-inner-secret-leaked-key");
            ...
            var allLogs = string.Join("\n", _capturedLogs);
            allLogs.Should().NotContain("pplx-inner-secret-leaked-key");
        }

        [Fact]
        public async Task GetRecommendations_WithNestedInnerExceptions_DoesNotLogApiKey()
        {
            var deepestException = new Exception("Auth failed: api_key=pplx-deepest-secret-789");
            var middleException = new Exception("Request error with pplx-middle-secret-456", deepestException);
            ...
            result.Should().BeEmpty();
            var allLogs = string.Join("\n", _capturedLogs);
            allLogs.Should().NotContain("pplx-deepest-secret-789");
            allLogs.Should().NotContain("pplx-middle-secret-456");
        }
