...
{
    /// <summary>
    /// Wave-7C characterization tests: verifies <see cref="BrainarrZaiGlmProvider"/>
    /// integrates correctly with <see cref="LlmAuthCircuit"/> (4-pact).
    ///
    /// <para>
    /// ZaiGlm speaks the OpenAI Chat Completions wire format; its CtorId is "zaiglm".
    /// </para>
    /// </summary>
    [Trait("Category", "Unit")]
    public sealed class BrainarrZaiGlmAuthCircuitAdoptionTests
    {
        private static readonly Logger Logger = LogManager.GetCurrentClassLogger();

        private const string ApiKey = "zai-test-glm-key-abc123";
        ...
        private static BrainarrZaiGlmProvider MakeProvider(
            FakeHttpClient http,
            LlmAuthCircuit circuit)
            => new BrainarrZaiGlmProvider(http, Logger, ApiKey, model: null, streamingExecutor: null, authCircuit: circuit);
        ...
        public async Task CompleteAsync_OpenCircuit_ThrowsImmediately()
        {
            ...
            for (int i = 0; i < 3; i++) circuit.RecordAuthFailure("zaiglm", ApiKey);
        ...
        public async Task CompleteAsync_OnHttp401_CallsRecordAuthFailure()
        {
            ...
            circuit.IsOpen("zaiglm", ApiKey, out _)
        ...
        public async Task CompleteAsync_OnSuccess_CallsRecordSuccess()
        {
            ...
            for (int i = 0; i < 3; i++) circuit.RecordAuthFailure("zaiglm", ApiKey);
            circuit.RecordSuccess("zaiglm", ApiKey); // allow one probe through
