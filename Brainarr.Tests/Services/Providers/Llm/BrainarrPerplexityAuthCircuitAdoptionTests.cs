...
{
    /// <summary>
    /// Wave-7C characterization tests: verifies <see cref="BrainarrPerplexityProvider"/>
    /// integrates correctly with <see cref="LlmAuthCircuit"/> (4-pact).
    /// </summary>
    [Trait("Category", "Unit")]
    public sealed class BrainarrPerplexityAuthCircuitAdoptionTests
    {
        private static readonly Logger Logger = LogManager.GetCurrentClassLogger();

        private const string ApiKey = "pplx-test-key-abc123";
        ...
        private static BrainarrPerplexityProvider MakeProvider(
            FakeHttpClient http,
            LlmAuthCircuit circuit)
            => new BrainarrPerplexityProvider(http, Logger, ApiKey, model: null, streamingExecutor: null, authCircuit: circuit);
        ...
        public async Task CompleteAsync_OpenCircuit_ThrowsImmediately()
        {
            ...
            for (int i = 0; i < 3; i++) circuit.RecordAuthFailure("perplexity", ApiKey);
        ...
        public async Task CompleteAsync_OnHttp401_CallsRecordAuthFailure()
        {
            ...
            circuit.IsOpen("perplexity", ApiKey, out _)
        ...
        public async Task CompleteAsync_OnSuccess_CallsRecordSuccess()
        {
            ...
            for (int i = 0; i < 3; i++) circuit.RecordAuthFailure("perplexity", ApiKey);
            circuit.RecordSuccess("perplexity", ApiKey); // allow one probe through
        ...
        public async Task CompleteAsync_OnHttp500_DoesNotPoisonCircuit()
        {
            ...
            // Wave-25 fix: the contract is "non-auth errors do NOT poison the circuit" —
            // the exception TYPE is incidental (Lidarr's IHttpClient may throw the parent
            // ProviderException before the provider can map to LlmProviderException). The
            // critical assertion is the gate state below.
