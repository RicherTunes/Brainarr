...
{
    /// <summary>
    /// Wave-7C characterization tests: verifies <see cref="BrainarrOpenRouterProvider"/>
    /// integrates correctly with <see cref="LlmAuthCircuit"/> (4-pact).
    /// </summary>
    [Trait("Category", "Unit")]
    public sealed class BrainarrOpenRouterAuthCircuitAdoptionTests
    {
        private static readonly Logger Logger = LogManager.GetCurrentClassLogger();

        private const string ApiKey = "sk-or-test-openrouter-key";
        ...
        private static BrainarrOpenRouterProvider MakeProvider(
            FakeHttpClient http,
            LlmAuthCircuit circuit)
            => new BrainarrOpenRouterProvider(http, Logger, ApiKey, model: null, streamingExecutor: null, authCircuit: circuit);
        ...
        public async Task CompleteAsync_OpenCircuit_ThrowsImmediately()
        {
            ...
            for (int i = 0; i < 3; i++) circuit.RecordAuthFailure("openrouter", ApiKey);
        ...
        public async Task CompleteAsync_OnSuccess_CallsRecordSuccess()
        {
            ...
            for (int i = 0; i < 3; i++) circuit.RecordAuthFailure("openrouter", ApiKey);
            circuit.RecordSuccess("openrouter", ApiKey); // allow one probe through
