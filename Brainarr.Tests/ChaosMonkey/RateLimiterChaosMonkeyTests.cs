...
namespace Brainarr.Tests.ChaosMonkey
{
    ...
    public class RateLimiterChaosMonkeyTests
    {
        ...
        {
            // Arrange — 5 requests at 5/sec means all fit within the first bucket (zero
            // queueing), eliminating thread-pool starvation under full-suite load.
            var rateLimiter = new RateLimiter(_logger);
            rateLimiter.Configure("stress", 5, TimeSpan.FromSeconds(1));
            ...
            completed.Should().Be(5, "All requests should eventually complete");
