...
- Go to GitHub Releases and download the ZIP asset for the latest version (currently v1.7.0).
...
- The repository maintains a tag named `latest` that points to the newest stable tag (presently v1.7.0). Any automation that follows releases/latest will pick up the most recent stable build automatically when we publish a new version.

Notes

- Brainarr requires Lidarr 3.1.2.4913+ on the plugins/nightly branch. Our CI extracts assemblies from the plugins Docker image and checks the package's declared minimum against its compiled host references.
...
1. Confirm Lidarr is on the nightly branch at version 3.1.2.4913 or newer by visiting `Settings → General → Updates`.
...
4. Download the Brainarr v1.7.0 release archive from GitHub.
...
**Ecosystem version contract:** Brainarr tracks `commonVersion: 1.18.0-dev`. Gitea runs the Common submodule pin guard plus the shared Common lint runner; the runner includes full ecosystem parity checks so version metadata and structural plugin contracts cannot silently drift. See [Common's ECOSYSTEM_VERSION_CONTRACT.md](https://github.com/RicherTunes/Lidarr.Plugin.Common/blob/main/docs/ECOSYSTEM_VERSION_CONTRACT.md) for details.
