...
- Go to GitHub Releases and download the ZIP asset for the latest version (currently v1.5.7).
...
- The repository maintains a tag named `latest` that points to the newest stable tag (presently v1.5.7). Any automation that follows
...
| Claude Code | Subscription | ✅ Verified in v1.5.7 | Uses local Claude Code CLI credentials (~/.claude/.credentials.json) |
| OpenAI Codex | Subscription | ✅ Verified in v1.5.7 | Uses local Codex CLI credentials (~/.codex/auth.json) |
<!-- PROVIDER_MATRIX_END -->
