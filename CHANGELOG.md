...
### Fixed (styles — Music Styles search is now token-order-independent — 2026-05-31)

- **Typing a multi-word style in a different word order than the catalog returned no options, so nothing could be selected.** The `Music Styles` field is a `TagSelect` whose autocomplete calls `styles/getoptions` → `StyleCatalogService.Search`, and `Search`'s scorer only did whole-string `StartsWith`/`Contains` on each entry's name + aliases. So `"Rock Alternative"` was checked as a *substring* of `"Alternative Rock"` — no match — and the dropdown came up empty, so the user couldn't add the style (it never "stuck"). Added a **token-AND (order-independent)** fallback to the scorer: for multi-word queries, an entry matches when **every** whitespace token appears in its name (or, lower-ranked, its name+aliases). `"Rock Alternative"` now finds `Alternative Rock`; `"alt rock"`, `"metal progressive"`, etc. likewise resolve. Ranked strictly **below** the existing direct prefix/contains matches so precise typing still ranks first, and only triggers for >1 token (single-token behavior is unchanged — every result still contains the query in name/alias). Token-**AND**, not OR: a query token the entry lacks still excludes it (e.g. plain `Rock` is not returned for `"Rock Alternative"`). Pinned by tests: reordered query finds the style, and the missing-token guard. Full suite green (3186 passed / 0 failed, excluding quarantined). Found by live-usage report (F4).
