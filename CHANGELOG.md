...
- Re-pin `ext/Lidarr.Plugin.Common` to `771bd3a18efd712cace51f949eac7823d29d247e`; `commonVersion`: `1.18.0-dev` -> `1.18.0`.
...
- Adopt shared monotonic retry budgets and overflow-safe exponential backoff; clock corrections no longer change elapsed retry admission.
...
- **New `dependency-scan` CI job** on the authoritative Gitea pipeline (mirrored in the guarded GitHub workflow, `github.com`-guarded): runs Common's `ext/Lidarr.Plugin.Common/scripts/ci/check-vulnerable-packages.ps1`, once per brainarr-owned csproj (`Brainarr.Plugin`, `Brainarr.Tests`, `Brainarr.Parity.Tests`, `Brainarr.Providers.OpenAI.Tests`, `Brainarr.TestKit.Providers`), failing the build when any direct or transitive NuGet package carries a known High/Critical vulnerability (fail-closed on unparseable output). Deliberately scoped to the brainarr-owned csprojs rather than `Brainarr.sln` — the solution also carries the three `ext/Lidarr.Plugin.Common` submodule projects, which Common polices in its own CI; scanning the sln here would fail brainarr's gate on a Common-side finding brainarr doesn't own. Local baseline scan at adoption time (all five targets): clean — no vulnerable packages, direct or transitive.
...
- Provider quirks now live visibly as template-hook overrides in the provider that owns them: OpenRouter's `HTTP-Referer`/`X-Title` headers + fixed cheap health-probe model + `routed_model` metadata, Perplexity's citation stripping (completion + stream chunks) + never emitting `response_format`, DeepSeek's `reasoning_content`, ZaiGlm's `MapZaiHttpError` (429 code-1113 → QuotaExceeded, still shared with ZaiCoding), OpenAI's 0.8 default temperature + REQUEST_START debug line.
- New `OpenAiChatProviderBaseContractTests` (15 tests, TDD'd before the base) pin the hook contract: the `SendsTemperature` policy hook (`Temperature_OmittedWhenPolicySaysNo` — guards any future provider whose endpoint rejects `temperature`, per the Z.AI Coding [1210] lesson), `SupportsJsonResponseFormat`, extra-header injection, `MapHttpError` override precedence, auth-circuit latch/pre-flight, the raw-body salvage fallback, and an exact-JSON pin of the serialized body shape (`model, messages, temperature, max_tokens, stream`).
- Deliberately NOT migrated: `BrainarrZaiCodingProvider` (Anthropic wire format on a raw HttpClient, opposite temperature contract) and `BrainarrOpenAiCompatibleProvider` (optional auth with a conditional circuit, GET `/v1/models` health check with Degraded semantics, ProviderOptions-based opt-in JSON mode, no streaming).
- Only intentional behavior nuance: OpenAI's non-streaming body builder treated an empty-string (non-null) `SystemPrompt` as present, emitting an empty `system` message; the shared builder uses the `IsNullOrEmpty` check the other five providers (and OpenAI's own streaming path) already used. No test pinned the old edge and no caller sends an empty-string system prompt.

### Fixed (exclusions — a "Never again" artist no longer leaks back through the recommendation cache — 2026-07-10)

- **A cache hit bypassed every hard-exclusion gate.** `RecommendationCoordinator.RunAsync` returned cached delivered items directly, but all "Never again" / strong-dislike enforcement (`RecommendationPipeline.FilterHardExcludedRecommendations`, `SafetyGateService`, the top-up/final `FilterHardExcluded` sweeps) lives inside `_pipeline.ProcessAsync`, which a cache hit skips — and the cache key (`RecommendationCacheKeyBuilder`) does not include dislike state, nor does `MarkAsDisliked` invalidate the cache. So: run 1 delivers and caches [X, Y, Z] → user marks X "Never again" → any run within the cache TTL (default 6h) re-delivered X, violating the documented "can neither be delivered nor pollute the review queue" guarantee. Fixed by re-applying the pipeline's own filter (`RecommendationPipeline.FilterHardExcluded`, same `RecommendationHistory.IsHardExcluded` full-key-equality predicate) to the cached list before returning it, with a Debug log of the removed count; a cache hit with no exclusions returns the cached list unchanged. TDD: RED test proved the leak (cached "Blocked Artist" returned after `MarkAsDisliked(..., NeverAgain)`), plus a no-over-filtering guard.
- **`GetExclusionPrompt` truncated artists whose name contains `'|'`.** The prompt hints (`EXCLUDE`/`AVOID`/`NEVER_RECOMMEND`/`DO_NOT_SUGGEST`) recovered artist names from exclusion keys via `Split('|')[0]`, so "AC|DC" became "ac" (keys encode `artist|album`, and an artist-level key like `ac|dc` is not recoverable by ANY split — last-pipe included). Now derives the names from the underlying history records (the source of truth), normalized with the same `NormalizeKeyPart` the keys use, so output is unchanged for pipe-free artists and full names are carried for pipe-bearing ones.
- Corrected the `RecommendationCache` constructor comment that claimed a null logger was historically tolerated by "never-hit log paths" — `TryGet`/`Set`/`Clear` call `_logger` unconditionally, so a null logger NREs on first use. Comment-only; no behavior change.

### Added (exclusions undo — `exclusions/get` + `exclusions/remove` UI actions — 2026-07-03, #62)

- **"Never again" is no longer one-way.** `RecommendationHistory.RemoveDislike` existed fully implemented and unit-tested but had zero production callers — the only way to undo a `review/never` was hand-editing `recommendation_history.json` inside the running container. Two new UI actions in `BrainarrOrchestrator.HandleAction` close the gap: `exclusions/get` returns `RecommendationHistory.GetExclusions()` directly (only normalized `artist|album` keys — nothing to redact), and `exclusions/remove` (required `artist`, optional `album`) calls `RemoveDislike`, now returning `bool` so the action can answer `{ ok: true, found: bool }` — removing a non-existent entry is an idempotent no-op, never an error.
- **Strong/NeverAgain dislikes are now enforced as deterministic hard exclusions in the live pipeline** (previously prompt-level hints only): filtered before MBID enrichment (`RecommendationPipeline`), re-gated in `SafetyGateService` before review-queue routing, and swept again on top-up/final import-list items. Matching uses `RecommendationHistory.GetKey` normalization with exact key equality (both album-specific and artist-level keys), never substring/prefix matching. Normal (non-strong) dislikes remain soft prompt hints.
- See CLAUDE.md → "Exclusions undo action" for the full contract. (The follow-up cache-hit leak fix shipped separately as #70, above.)

### Added (cost visibility — `cost/get` action wired to `TokenCostEstimator` — 2026-07-02, #59/#60)

- **`TokenCostEstimator` was fully built + tested but had zero production callers; it is now wired into the real recommendation path** so cloud-provider users can see run cost via a new `cost/get` UI action (optional `days` lookback param, default 30, clamped to `[1, 365]`). Tracking fires at BOTH real HTTP round-trip sites — `RecommendationGenerator.GenerateRecommendationsAsync` (initial batch) and `IterativeRecommendationStrategy.GetIterativeRecommendationsAsync` (top-up, which makes its own provider calls) — exactly one `TrackUsage` per provider call; missing the top-up site silently undercounted every run that tops up (caught by a RED test expecting 2, getting 1). Both sites translate the UI's friendly model label to the raw API id via the new `ModelIdMapper.ToRawId(AIProvider, string)` overload before tracking, otherwise almost every cloud model reports as unpriced (the RED test literally received `"GPT4o_Mini"` instead of `"gpt-4o-mini"`).
- **Pricing-table honesty: refresh + honest-unknown, never a fabricated dollar figure.** The old table silently defaulted any unrecognized model to a hardcoded $0.001/1K. `GetModelPricing` now returns `null` for anything it can't match; estimates carry `IsPriceKnown=false` with `EstimatedCost=0` and an explanatory breakdown, and `UsageStatistics` reports `UnpricedRequestCount` alongside `TotalCost` (which only ever sums priced entries). Local/subscription/CLI providers are a KNOWN $0 (no metered per-token cost); Z.AI providers and OpenRouter are deliberately unpriced rather than guessed. The follow-up (#60) fail-closed the model-prefix partial matching so a newer, differently-priced sibling model (e.g. `claude-sonnet-4-6`) can't inherit an older dated model's price.

### Fixed (healer wave-1 — real freshness, storage-root outage coalescing, unified redaction — 2026-07-02, #56/#57/#58)

- **Freshness is now computed, not hardcoded.** Treatment-plan freshness was hardcoded "current" at projection time, making the whole Freshness dimension inert. `HealerFreshnessEvaluator` now computes it read-only at scan time by comparing Lidarr's recorded Size/Modified against the live on-disk fingerprint the scan already takes (unchanged→current, drifted→stale, gone→missing, unprobeable→unknown), persisted on the finding and consumed by `HealerTriageAdvisor`. The follow-up (#57) made persisted freshness **fail closed**: values are allowlisted, and missing/malformed/stale/hand-edited values normalize to `unknown` + human review instead of being trusted.
- **One offline storage root no longer floods the queue with thousands of per-file findings.** The scan derives each path-unresolved finding's storage root (drive / UNC share / mount via `HealerStorageRoot`), and when enough findings share a root whose bounded health precheck reports it offline, collapses them into ONE `STORAGE_ROOT_OFFLINE` `PathInconsistency` finding carrying `affectedTrackCount` (the blast radius). Bounded: at most `MaxRootHealthProbes` distinct roots probed, one bounded probe each; a healthy root leaves per-file findings untouched. The follow-up (#58) hardened the root probing with `StorageRootAvailabilityProbe` — TTL-cached probe results (bounded cache, capped concurrent probes, per-probe timeout) with a later fix removing sync-over-async from the probe path.
- **Unified the token-redaction predicate across the store and action boundaries.** `LibraryHealerFindingStore` (disk) and `LibraryHealerActionHandler` (API) had diverged duplicate `ShouldRedactTokenMaterial` predicates — the disk copy lacked the metadata/command-material checks, so e.g. a bare MusicBrainz id in a token field was redacted in the API response but persisted raw. Both boundaries now share one predicate (`LibraryHealerTokenRedaction`).
- All three preserve the A2 read-only invariant (`LibraryHealerReadOnlyArchitectureTests` stays green): the healer still only detects and records redacted evidence. See `docs/library-healer.md`.

### Added (OpenAI Codex: real ChatGPT-subscription support + token rotation — ported from GitHub PR #648 by maliavko, 2026-08-25; adapted to the primary line 2026-09-06)

- **`OpenAICodexSubscription` now works with a plain ChatGPT subscription** (no API key). Previously only the `OPENAI_API_KEY`-in-`auth.json` path worked: a `codex auth login` OAuth token 401s against `api.openai.com/v1/chat/completions`, so subscription users got a dead provider and an empty model dropdown. The provider is now dual-mode — an OAuth session is dispatched to the ChatGPT backend **Responses API** (`POST https://chatgpt.com/backend-api/codex/responses`, `Authorization: Bearer` + `chatgpt-account-id` + `OpenAI-Beta` + `originator: codex_cli_rs` + `codex_cli_rs/<ver>` UA + `session_id`, `stream:true`, SSE reconstructed by the new `CodexSseParser`), while an `OPENAI_API_KEY` keeps the legacy Platform `chat/completions` path. Dispatched via a raw `HttpClient` because `ManagedHttpDispatcher` rejects the non-Lidarr UA/originator the backend requires (same shape as `BrainarrZaiCodingProvider`).
- **Automatic token rotation** (`CodexTokenRefresher`, `CodexJwt`). The access token is a ~10-day JWT and `auth.json` has no `expires_at`, so expiry is read from the JWT `exp` claim. On 401/403 the provider performs the OAuth2 refresh-token grant against `https://auth.openai.com/oauth/token` (Codex CLI public client id) and retries once, writing the rotated `access_token`/`id_token`/`refresh_token` back into `~/.codex/auth.json` **atomically** (temp + `File.Move`, `0600` on POSIX) with every other field (`auth_mode`, `OPENAI_API_KEY`, `tokens.account_id`) preserved, so the Codex CLI keeps working. Refresh runs on **both** `CompleteAsync` and `CheckHealthAsync`.
- **Model dropdown populated** for this provider (it rendered empty, leaving the effective model unset). `BrainarrConstants.OpenAICodexModels` is the single source of truth shared by the dropdown and the provider's coercion: `gpt-5.6-terra` (new default, was `gpt-4o`), `gpt-5.6-sol`, `gpt-5.6-luna`, `gpt-5.5`. `gpt-5.4`/`-mini` are deliberately excluded (they leave Codex 2026-08-31) and `gpt-5.3-codex-spark` is ChatGPT Pro only.
- **Escapes a settings-UI deadlock.** Lidarr does not refetch the field schema when only the *Provider* dropdown changes, so switching to Codex left the previous provider's model in the field → `400 "model is not supported"` → Test fails → Lidarr refuses to Save → the user could never save Codex to get the refreshed dropdown. `NormalizeCodexModel` coerces an unrecognised slug to the default (logging a Warn) so Test passes, Save works, and the correct models appear on reopen.
- Live-verified end-to-end by a Plus subscriber against a real Lidarr `3.1.2.4913` host: plugin loads, Test green, sync returns recommendations.
- **Operator note:** these are reasoning models (~13–15s for a full list) and this backend **rejects `max_output_tokens`** (`400 Unsupported parameter`), so the timeout-aware output budget cannot be sent and length is bounded only by the request timeout. Raise **AI Request Timeout** to **60s+**; the default 30s times out at 0 items. `LlmCapabilityFlags.JsonMode` is deliberately not advertised (the backend only accepts `text.format=json_object` when the input message contains the word "json").
### Fixed (perf — finish the `Album.ArtistId` / `Album.ArtistMetadata` N+1 lazy-load sweep — 2026-07-03)

- **Completes the OOM sweep started above.** The initial fix closed the reported crash's call path (`DuplicateFilterService`/`LibraryContextBuilder`/`StyleContextBuilder`); the remaining `Album.ArtistId`-in-a-loop sites that shared the identical per-album `LazyLoaded<Artist>` → per-row `ArtistRepository.Query()` DB round trip (N+1 → OOM at ~11,700-artist scale) are now all swept to the same `ArtistMetadataId` pattern:
  - `DefaultSamplingService.SampleArtists` — **live-caught OOM site on the prompt path** (degraded to the baseline prompt when it OOM'd). Its album-count grouping now keys on `ArtistMetadataId` (counts translated back to `Artist.Id` via the matches' already-materialized artists); `CreateSampleAlbum`/`ResolveArtistName` resolve each sampled album's artist id + name from an `ArtistMetadataId -> Artist` map built once from `allArtists`, never touching `Album.ArtistId`/`Album.Artist.Value`/`Album.ArtistMetadata.Value`.
  - `LibraryAnalyzer.AnalyzeCollectionDepth` + `GetTopArtistsByAlbumCount` — group on `ArtistMetadataId`; `TopCollectedArtists.ArtistId` still reports the real `Artist.Id` via an `ArtistMetadataId -> Artist.Id` translation map, so counts/ids are unchanged.
  - `AdvancedDuplicateDetector` + `SimpleRecommendationValidator` — build an `ArtistMetadataId -> albums` lookup **once** instead of `albums.Where(a => a.ArtistId == artist.Id)` re-scanned per candidate artist (was O(candidates × albums) lazy loads).
  - `IterativeRecommendationStrategy.BuildExistingAlbumsSet` (top-up dedup baseline) — resolved the artist name via `Album.ArtistMetadata.Value.Name` for **every** album; `Album.ArtistMetadata` is the **sibling** per-album lazy-load (also unloaded by `GetAllAlbums()`), so this was the same OOM mechanism. Now resolves the name from the `allArtists` list keyed on `ArtistMetadataId`.
- TDD: adds `RecordingArtistMetadataLazyLoaded` (sibling `LazyLoaded<ArtistMetadata>` double) to `AlbumArtistLazyLoadTestDoubles.cs` and four new RED→GREEN suites (`LibraryAnalyzerNPlusOneTests`, `DefaultSamplingServiceNPlusOneTests`, `ValidationNPlusOneTests`, `IterativeRecommendationStrategyNPlusOneTests`) that prove the per-album DB round trip count scales with album count when unfixed (5 / 14 / 23 / 123 / 150 / 400 round trips observed) and is exactly **0** once fixed. A handful of existing tests that had encoded the old lazy resolution (empty `allArtists` + album-lazy-`Artist`/`ArtistMetadata`) were updated to the production data shape (artist supplied via `allArtists`, linked by `ArtistMetadataId`). Reviewed-and-left as safe (no host `Album` lazy read): the `.ThenBy(a => a.ArtistId)` tiebreaks in `LibraryPromptPlanner`/`LibraryPromptRenderer` and the `ArtistId` reads on `LibrarySampleArtist`/`LibrarySampleAlbum`/`ArtistDepth`/`LibraryHealer*` (all plain-int model/record fields). Full suite green (3,415 passed / 0 failed / 9 skipped, excluding quarantined stress tests).
